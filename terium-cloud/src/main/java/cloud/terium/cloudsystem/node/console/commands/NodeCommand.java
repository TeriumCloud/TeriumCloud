    ...
    public void execute(String[] args) {
        if (args.length >= 1) {
            switch (args[0]) {
                case "execute" -> {
                    if (args.length > 1) {
                        Optional<INode> cloudNode = NodeStartup.getNode().getNodeProvider().getNodeByName(args[1]);
                ...
                case "add" -> {
                    if (args.length >= 5) {
                        try {
                            NodeStartup.getNode().getNodeFactory().createNode(args[1], args[4], new InetSocketAddress(args[2], Integer.parseInt(args[3])));
                ...
                case "remove" -> {
                    if (args.length > 1) {
                        ...
                        cloudNode.ifPresentOrElse(node -> {
                            ...
                            NodeStartup.getNode().getNodeFactory().deleteNode(node);
                ...
                case "list" -> {
                    Logger.log(NodeStartup.getNode().getNodeProvider().getAllNodes().size() > 0 ? "All registered nodes:" : "There is no registered node.", LogType.INFO);
                    NodeStartup.getNode().getNodeProvider().getAllNodes().stream().filter(node -> node != NodeStartup.getNode().getThisNode()).forEach(node -> Logger.log("Name: " + node.getName() + "§7(" + (node.isConnected() ? "§aCONNECTED" : "§cNOT CONNECTED") + "§7)§f - Address: " + node.getAddress().getHostName() + ":" + node.getAddress().getPort(), LogType.INFO));
                    return;
                }
            }
        ...
        switch (args.length) {
            ...
            case 2 -> {
                if (args[0].equalsIgnoreCase("remove") || args[0].equalsIgnoreCase("execute") || args[0].equalsIgnoreCase("info")) {
                    return NodeStartup.getNode().getNodeProvider().getAllNodes().stream().filter(node -> node != NodeStartup.getNode().getThisNode()).map(INode::getName).toList();
                }
            }
            case 3 -> {
