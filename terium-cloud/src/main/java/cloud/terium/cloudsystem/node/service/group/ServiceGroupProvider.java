...
public class ServiceGroupProvider implements ICloudServiceGroupProvider {
        ...
        switch (ServiceType.valueOf(serviceGroup.get("servicetype").getAsString())) {
            case Proxy -> {
                ICloudServiceGroup iCloudServiceGroup = new DefaultProxyGroup(serviceGroup.get("group_name").getAsString(),
                        ...
                        NodeStartup.getNode().getNodeProvider().getNodeByName(serviceGroup.get("node").getAsString()).orElseGet(null),
                        new LinkedList<>(NodeStartup.getNode().getNodeProvider().getAllNodes().stream().filter(node -> serviceGroup.get("fallback_nodes").getAsJsonArray().toString().contains(node.getName())).toList()),
                        new LinkedList<>(NodeStartup.getNode().getTemplateProvider().getAllTemplates().stream().filter(template -> serviceGroup.get("templates").getAsJsonArray().toString().contains(template.getName())).toList()),
    ...
    public void updateServiceGroup(ICloudServiceGroup serviceGroup) {
        ...
        try (InputStreamReader reader = new InputStreamReader(Files.newInputStream(file.toPath()), StandardCharsets.UTF_8)) {
            ...
            NodeStartup.getNode().getEventProvider().callEvent(new GroupUpdateEvent(serviceGroup.getGroupName()));
        } catch (IOException ex) {
            ex.printStackTrace();
        }
    }
    ...
    public int getOnlineServicesFromServiceGroup(String groupName) {
        return NodeStartup.getNode().getServiceProvider().getCloudServicesByGroupName(groupName).size();
    }
