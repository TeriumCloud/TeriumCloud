                                        ...
                                        protected void channelRead0(ChannelHandlerContext channelHandlerContext, Object packet) {
                                            try {
                                                ...
                                                if (packet instanceof PacketPlayOutServiceRegister newPacket) {
                                                    // Nodes
                                                    NodeStartup.getNode().getNodeProvider().getAllNodes().forEach(node -> channelHandlerContext.channel().writeAndFlush(
                                                            new PacketPlayOutNodeAdd(node.getName(), node.getKey(), node.getAddress(), node.getMaxMemory(), node.isConnected())));

                                                    // Templates
                                                    NodeStartup.getNode().getTemplateProvider().getAllTemplates().forEach(template -> channelHandlerContext.channel().writeAndFlush(
                                                            new PacketPlayOutTemplateAdd(template.getName(), template.getPath().toString())));

                                                    // Groups
                                                    NodeStartup.getNode().getServiceGroupProvider().getAllServiceGroups().forEach(group -> channelHandlerContext.channel().writeAndFlush(
                                                    ...
                                                    // Players
                                                    NodeStartup.getNode().getCloudPlayerProvider().getOnlinePlayers().forEach(player -> channelHandlerContext.channel().writeAndFlush(
                                                    ...
                                                    // Services
                                                    NodeStartup.getNode().getServiceProvider().getAllCloudServices().forEach(cloudService -> channelHandlerContext.channel().writeAndFlush(
                                                    ...
                                                    NodeStartup.getNode().getServiceProvider().getAllCloudServices().stream().filter(cloudService -> cloudService.getServiceState().equals(ServiceState.ONLINE)).toList().forEach(cloudService ->
                                                    ...
                                                    // Modules
                                                    NodeStartup.getNode().getModuleProvider().getAllModules().forEach(module -> channelHandlerContext.channel().writeAndFlush(
