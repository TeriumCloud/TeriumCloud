...
public class TemplateCommand extends Command {
            ...
            switch (args[0]) {
                case "create" -> {
                    if (args.length > 1) {
                        ...
                        NodeStartup.getNode().getTemplateFactory().createTemplate(args[1]);
                ...
                case "info" -> {
                    if (args.length > 1) {
                        Optional<ITemplate> cloudTemplate = NodeStartup.getNode().getTemplateProvider().getTemplateByName(args[1]);
                ...
                case "list" -> {
                    ...
                    NodeStartup.getNode().getTemplateProvider().getAllTemplates().forEach(template -> Logger.log("Name: " + template.getName() + " - Path: " + template.getPath().toString(), LogType.INFO));
                    return;
                }
            }
        }
    ...
    public List<String> tabComplete(String[] args) {
        if (args.length == 1) {
            ...
            return NodeStartup.getNode().getTemplateProvider().getAllTemplates().stream().map(ITemplate::getName).toList();
        }
