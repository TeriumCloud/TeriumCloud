...
public class ServiceCreateEvent extends Event {
    ...
    public ServiceCreateEvent(String serviceName, int serviceId, int port, int maxPlayers, int memory, INode node, ICloudServiceGroup serviceGroup, List<ITemplate> templates, HashMap<String, Object> propertyCache) {
        ...
        this.port = port;
        TeriumCloud.getTerium().getNetworking().sendPacket(new PacketPlayOutCreateService(serviceName, serviceId, port, maxPlayers, memory, node.getName(), serviceGroup.getGroupName(), templates.stream().map(ITemplate::getName).toList(), propertyCache));
    }
}