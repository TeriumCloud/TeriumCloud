                ...
                if (args.length > 1) {
                    if (args[1].equalsIgnoreCase("lobby") || args[1].equalsIgnoreCase("server")) {
                        if (args.length == 6 || args.length == 7) {
                            switch (args[1]) {
                                case "lobby" ->
                                        ClusterStartup.getCluster().getServiceGroupProvider().registerServiceGroup(ClusterStartup.getCluster().getServiceGroupFactory().createLobbyGroup(args[2], "Default lobby group", args.length == 6 ? ClusterStartup.getCluster().getThisNode() : ClusterStartup.getCluster().getNodeProvider().getNodeByName(args[6].replace("-node=", "")).orElseGet(null), List.of(ClusterStartup.getCluster().getTemplateFactory().createTemplate(args[2])), args[3], false, Boolean.parseBoolean(args[4]), 20, Integer.parseInt(args[5]), 1, 1));
                                case "server" ->
                                        ClusterStartup.getCluster().getServiceGroupProvider().registerServiceGroup(ClusterStartup.getCluster().getServiceGroupFactory().createServerGroup(args[2], "Default server group", args.length == 6 ? ClusterStartup.getCluster().getThisNode() : ClusterStartup.getCluster().getNodeProvider().getNodeByName(args[6].replace("-node=", "")).orElseGet(null), List.of(ClusterStartup.getCluster().getTemplateFactory().createTemplate(args[2])), args[3], false, Boolean.parseBoolean(args[4]), 20, Integer.parseInt(args[5]), 1, 1));
                            }
