            ...
            ClusterStartup.getCluster().getServiceGroupProvider().getServiceGroupByName(args[0]).ifPresentOrElse(serviceGroup -> {
                if (ClusterStartup.getCluster().getThisNode().getName().equals(serviceGroup.getGroupNode().getName()))
                    if(ClusterStartup.getCluster().getServiceFactory().containsServiceGroup(serviceGroup)) {
                        ...
                        TeriumAPI.getTeriumAPI().getProvider().getEventProvider().callEvent(new ServiceCreateEvent(serviceGroup.getGroupName(), ClusterStartup.getCluster().getServiceProvider().getFreeServiceId(serviceGroup), serviceGroup.hasPort() ? (ClusterStartup.getCluster().getServiceProvider().getAllServices().stream().filter(cloudService -> cloudService.getServiceGroup().hasPort()).toList().isEmpty() ? serviceGroup.getPort() : (serviceGroup.getPort() + ClusterStartup.getCluster().getServiceProvider().getAllServices().stream().filter(cloudService -> cloudService.getServiceGroup().hasPort()).toList().size())) : -1, serviceGroup.getMaxPlayers(), serviceGroup.getMemory(), serviceGroup.getGroupNode(), serviceGroup, serviceGroup.getTemplates(), new LinkedHashMap<>(), "group_only"));
                    }
                else {
                    ClusterStartup.getCluster().getNodeProvider().getClientFromNode(serviceGroup.getGroupNode()).writeAndFlush(new PacketPlayOutCreateService(serviceGroup.getGroupName(), -1, serviceGroup.hasPort() ? (ClusterStartup.getCluster().getServiceProvider().getAllServices().stream().filter(cloudService -> cloudService.getServiceGroup().hasPort()).toList().isEmpty() ? serviceGroup.getPort() : (serviceGroup.getPort() + ClusterStartup.getCluster().getServiceProvider().getAllServices().stream().filter(cloudService -> cloudService.getServiceGroup().hasPort()).toList().size())) : -1, serviceGroup.getMaxPlayers(), serviceGroup.getMemory(), serviceGroup.getGroupNode().getName(), serviceGroup.getGroupName(), serviceGroup.getTemplates().stream().map(ITemplate::getName).toList(), new HashMap<>(), "group_only"));
                }
