package cloud.terium.minestom.cloud.impl.pipe;

import cloud.terium.minestom.MinestomExtension;
import cloud.terium.minestom.cloud.TeriumExtension;
import cloud.terium.minestom.cloud.impl.module.LoadedModule;
import cloud.terium.minestom.cloud.impl.node.Node;
                        ...
                        if (!TeriumAPI.getTeriumAPI().getProvider().getThisService().getServiceType().equals(ServiceType.Proxy)) {
                            if (packet instanceof PacketPlayOutServiceExecuteCommand newPacket)
                                if (TeriumAPI.getTeriumAPI().getProvider().getThisService().getServiceName().equals(newPacket.cloudService()))
                                    MinestomExtension.getInstance().executeCommand(newPacket.command());
                        }
                    }
